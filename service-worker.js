const CACHE='dino-cat-dash-v9-2.1.2';
const FALLBACK='./index.html?v=2.1.2';
const CORE=[
  FALLBACK,
  './style.css?v=2.1.2',
  './ipad-layout.css?v=2.1.2',
  './manifest.webmanifest?v=2.1.2',
  './js/config.js?v=2.1.2',
  './js/audio.js?v=2.1.2',
  './js/render.js?v=2.1.2',
  './js/boss-visual-fix.js?v=2.1.2',
  './js/mechanics.js?v=2.1.2',
  './js/main.js?v=2.1.2'
];
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(response=>{const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(FALLBACK,copy));return response}).catch(()=>caches.match(FALLBACK)));return
  }
